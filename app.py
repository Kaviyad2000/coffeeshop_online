...
def submit():
    ...
    noofitem=request.form['noofitem']
    
    # Parse the appointment date
    appoint_date = datetime.strptime(date, '%Y-%m-%d')
    current_date = datetime.now()

    # Check if the appointment date is in the past
    if appoint_date < current_date:
    ...
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute("INSERT INTO \"order\" (name, mail, location, date, noofitem) VALUES (?, ?, ?, ?, ?)", (name, mail, location, date, noofitem))
    conn.commit()
    conn.close()
